    ...
    class WC_Gateway_Sagepay_Form extends WC_Payment_Gateway {
        ...
        public function generate_sagepay_form( $order_id ) {
            ...
            $sage_pay_args_array = array(
                ...
                'FailureURL'        => apply_filters( 'woocommerce_sagepayform_cancelurl', $order->get_cancel_order_url(), $order_id, $order_key ),
                ...
                'BillingFirstnames' => substr( ( $pre_wc_30 ? $order->billing_first_name : $order->get_billing_first_name() ), 0, $this->BillingFirstnames_length ),
                'BillingSurname'    => substr( ( $pre_wc_30 ? $order->billing_last_name : $order->get_billing_last_name() ), 0, $this->BillingSurname_length ),
                'BillingAddress1'   => substr( ( $pre_wc_30 ? $order->billing_address_1 : $order->get_billing_address_1() ), 0, $this->BillingAddress1_length ),
                'BillingAddress2'   => substr( ( $pre_wc_30 ? $order->billing_address_2 : $order->get_billing_address_2() ), 0, $this->BillingAddress2_length ),
                'BillingCity'       => $this->city( substr( ( $pre_wc_30 ? $order->billing_city : $order->get_billing_city() ), 0, $this->BillingCity_length ) ),
                'BillingState'      => substr( ( WC_Sagepay_Common_Functions::sagepay_state( $billing_country, $billing_state ) ), 0, $this->BillingState_length ),
                'BillingPostCode'   => substr( ( $pre_wc_30 ? $order->billing_postcode : $order->get_billing_postcode() ), 0, $this->BillingPostCode_length ),
                'BillingCountry'    => substr( ( $billing_country ), 0, $this->BillingCountry_length ),
                'BillingPhone'      => substr( ( $pre_wc_30 ? $order->billing_phone : $order->get_billing_phone() ), 0, $this->BillingPhone_length ),
                'CustomerEMail'     => substr( ( $pre_wc_30 ? $order->billing_email : $order->get_billing_email() ), 0, $this->CustomerEMail_length ),
                ...
                'DeliveryPhone'     => substr( ( apply_filters( 'woocommerce_sagepay_form_deliveryphone', $pre_wc_30 ? $order->billing_phone : $order->get_billing_phone(), $order ) ), 0, $this->DeliveryPhone_length ),
            ...
            if ( $this->status == 'testing' && $this->testvendorpwd ) {
           	...
           	$sagepaycrypt_b64  = WC_Sagepay_Common_Functions::encrypt( $sage_pay_args, $vendorpwd );
