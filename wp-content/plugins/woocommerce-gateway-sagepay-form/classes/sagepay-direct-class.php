<?php
    ...
    class WC_Gateway_Sagepay_Direct extends _WC_Gateway_Sagepay_Direct {
        ...
        public function __construct() {
			...
			$this->cvv_script			= isset( $this->settings['cvv_script'] ) && $this->settings['cvv_script'] == 'yes' ? true : false;
			...
			// Add 'Authorized' to needs_payment
		...
		public function validate_fields() {
			...
			try {
				 ...
				 */
				if( $this->cvv_script && $sage_card_token !== false && strtoupper($sage_card_type) != 'PAYPAL' ) {
        ...
        function authorise_3dsecure( $order_id ) {
        	...
        	// woocommerce order instance
           	...
           	if ( $order->needs_payment() ) {
		    	...
		    	// Order does not need to be paid for, redirect to checkout
				wp_redirect( $redirect_url );
				exit;
		    }
		...
		function build_query ( $order, $sage_card_number = '', $sage_card_exp_month = '', $sage_card_exp_year = '', $sage_card_cvc = '', $sage_card_type = '', $sage_card_save_token = false, $sage_card_token = false ) {
			...
			if( $sage_card_save_token && $registered_token ) {
				...
				if ( $token ) {
					if ( $token->get_user_id() == $customer_id ) {
		    			...
		    			// AVS and CVS=2 for token payments
						if( $sage_card_cvc != '' ) {
							$details["ApplyAVSCV2"]		= $this->cvv;
							$details["CV2"] 			= $sage_card_cvc;
							// $details["Apply3DSecure"]	= $this->secure;
						}

						// 3D Secure for token payments
			...
			$billing_city 			= $this->city( $pre_wc_30 ? $order->billing_city : $order->get_billing_city() );
			...
			$shipping_city 			= apply_filters( 'woocommerce_sagepay_direct_deliverycity', $this->city( WC_Sagepay_Common_Functions::check_shipping_address( $order, 'shipping_city' ) ), $order );
			...
			$this->basketoption = strtoupper($data["CardType"]) == 'PAYPAL' ? 1 : $this->basketoption;
    	...
    	function sagepay_credit_card_form() {
			...
			$fields = apply_filters( 'woocommerce_sagepaydirect_credit_card_form_fields', $fields, $this );
...
?>
		...
		function sagepaydirect_scripts() {
			...
			// if ( ! wp_script_is( 'jquery-payment', 'registered' ) ) {
				wp_register_script( 'jquery-payment', SAGEPLUGINURL.'assets/js/jquery.payment.js', array( 'jquery' ), '3.13.4', true );
			// }
    	...
    	function process_refund( $order_id, $amount = NULL, $reason = '' ) {
             ...
             * Convert the $data array for Sage
             */
            $api_request = http_build_query( $data, '', '&' );
		...
		public function needs_payment( $needs_payment, $woocommerce ) {
