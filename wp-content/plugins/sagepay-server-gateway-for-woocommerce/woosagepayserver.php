  ...
  if ( ! class_exists( 'WC_Payment_Gateway' ) ) { return; }
    ...
    public function generate_sagepayserver_form( $order_id ) {
      ...
      $response = wp_remote_post(
        ...
        if(isset($resp['Status'])) update_post_meta($order_id, 'Status', $resp['Status']);

        if(isset($resp['StatusDetail'])) update_post_meta($order_id, 'StatusDetail', $resp['StatusDetail']);

        if(isset($resp['VPSTxId'])) update_post_meta($order_id, 'VPSTxId', $resp['VPSTxId']);

        if(isset($resp['CAVV'])) update_post_meta($order_id, 'CAVV', $resp['CAVV']);

        if(isset($resp['SecurityKey'])) update_post_meta($order_id, 'SecurityKey', $resp['SecurityKey']);

        if(isset($resp['TxAuthNo'])) update_post_meta($order_id, 'TxAuthNo', $resp['TxAuthNo']);

        if(isset($resp['AVSCV2'])) update_post_meta($order_id, 'AVSCV2', $resp['AVSCV2']);

        if(isset($resp['AddressResult'])) update_post_meta($order_id, 'AddressResult', $resp['AddressResult']);

        if(isset($resp['PostCodeResult'])) update_post_meta($order_id, 'PostCodeResult', $resp['PostCodeResult']);

        if(isset($resp['CV2Result'])) update_post_meta($order_id, 'CV2Result', $resp['CV2Result']);

        if(isset($resp['3DSecureStatus'])) update_post_meta($order_id, '3DSecureStatus', $resp['3DSecureStatus']);

        if(isset($orderid)) update_post_meta($order_id, 'VendorTxCode', $orderid );
      ...
      if( isset( $_POST['VendorTxCode'] ) ){
				...
				if( $_POST['Status'] == 'OK' ){
					...
					$order->add_order_note( __('Sagepay Direct payment completed', 'patsatech-woo-sagepay-server') . ' ( ' . __('Transaction ID: ','patsatech-woo-sagepay-server') . strip_tags($_POST['VendorTxCode']) . ' )' );
          ...
          wc_add_notice( __('Aborted by user.', 'patsatech-woo-sagepay-server'), 'error' );
					...
					$params = array('Status' => 'INVALID', 'StatusDetail' => __('Transaction errored - ', 'patsatech-woo-sagepay-server') . strip_tags($_POST['StatusDetail']) );
          $redirect_url = $order->get_cancel_order_url();
				}else{
					$params = array('Status' => 'INVALID', 'StatusDetail' => __('Transaction failed - ', 'patsatech-woo-sagepay-server') . strip_tags($_POST['StatusDetail']) );
          $redirect_url = $order->get_cancel_order_url();
				}
			}else{
