        ...
        while ( ( $line = $csv_reader->get_row() ) !== NULL ) {
         ...
         //       continue;
            ...
            if ( is_wp_error( $user_id ) ) {
                ...
                $expDate1 = $usermeta['expiryDate1'];
                $expDate2 = $usermeta['expiryDate2'];
                
                date_default_timezone_set('Europe/London');

                
                
                $x = $membership->get_end_date();

                $e1t=str_replace("/","-",$expDate1);
                $e2t=str_replace("/","-",$expDate2);
                
                $e1unix=strtotime($e1t);
                $e2unix=strtotime($e2t);
                
            // which is later
                if ($e1unix>$e2unix){
                    $e2unix = $e1unix;
                }
                $new_end_date = date('Y-m-d',$e2unix);
                $new_end_date = date('Y-m-d', strtotime("+1 year", $e2unix));


                $membership->set_end_date( $new_end_date );
            }
            $user_ids[] = $user_id;
            $rkey++;
        }
